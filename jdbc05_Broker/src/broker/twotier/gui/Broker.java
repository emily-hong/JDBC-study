...
//		Database db = null;
	...
	*/
	...
	//=============================================================
		...
		// 실시간으로 주식정보를 뿌리는 쓰레드 추가하기
	public static void main(String args[])throws Exception { // 기본(메인) 쓰레드
		Broker broker = new Broker();		// -> proccess
		Thread t = new Thread(broker);
		t.start(); // 알아서 runnable 메모리 영역을 타고들어감
	}

	// Runnable 로 생성됨
	// 쓰레드가 작업하는 코드를 run() 안에다 구현
	// 3초마다 디비가서 상장된 주식의 정보를 가져오는 일만 전담하는 쓰레드를 작성
	@Override
	public void run() {
		while(true) { // 무한루핑을 돌면서... 3초마다 한번씩 디비의 모든 조식을 가져온다.
			try {
				showStockList(db.getAllStocks(), stockList); // 모든 주식정보를 가져와서 stockList로 뿌림
